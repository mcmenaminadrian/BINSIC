...
251 PAUSE 5
252 LET ZZ = 0
253 IF A(X,Y) = 2 THEN LET ZZ = 1
254 IF ZZ = 1 THEN LET A(X,Y)=0
255 IF ZZ = 1 THEN NEXT Y
257 IF A(X,Y) = 3 THEN LET A(X,Y)=1
258 IF A(X,Y) <> 1 THEN NEXT Y
261 PRINT TAB(Y), "*";
262 IF X<X3 THEN LET X3=X
264 IF X>X4 THEN LET X4=X
265 IF Y<Y3 THEN LET Y3=Y
266 IF Y>Y4 THEN LET Y4=Y
270 NEXT Y
290 NEXT X
...
560 NEXT J
570 NEXT I
580 IF A(X,Y) = 0 THEN GOSUB 1000
585 IF A(X,Y) = 0 THEN NEXT Y
590 IF C<3 OR C>4 THEN LET A(X,Y) = 2
592 IF C<3 OR C>4 THEN NEXT Y
595 LET P=P+1
620 NEXT Y
630 NEXT X
635 LET X1=X1-1
...
640 GOTO 210
650 END
1000 IF C = 3 THEN LET A(X,Y) = 3
1010 IF C = 3 THEN P=P+1
1020 RETURN
